...
Syspower.VerifyNumber = (function (initObj) {
    "use strict";
    var getParamValue = function (aValue, def) {
    ...
    var mCanvas, mContext, mBgCanvas, mBgContext, cWidth, cHeight;
    var aNumber; // 檢查碼
    // 基本設定 ====================================================================
    // 取得Component寬度+高度
    var aComponent = $("#" + ComponentId);
    //處理元件<span>抓不到width與height問題 
    aComponent.css({"display": "inline-block","position": "relative"})
    ...
    function drawBgToContext(mousePos, eventType) {
        // 將背景層貼到前幕
        ...
        checkNumber: function (number, callback) {
            ...
            setTimeout(function () {
                if (callback instanceof Function) {
                    ...
                    alert("第二個參數必須是callback function !");
                }
            }, 0);
        }
    };
